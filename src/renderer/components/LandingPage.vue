    ...
    <el-aside width="350px">
      <el-card class="box-card" shadow="always" style="margin: 1em;">
        <el-form ref="form" :model="form" label-width="70px" size="small" :disabled="loading">
          ...
          <el-button type="info" plain icon="el-icon-message" style="width: 100%;" size="small" @click="submit"></el-button>
        </el-form>
      </el-card>
    </el-aside>
    <el-main>
      ...
      <el-progress class="progress" v-if="loading" type="circle" :percentage="percentage" color="#3f51b5"></el-progress>
    </el-main>
  </el-container>
</template>
  ...
  export default {
    ...
    data() {
      return {
        ...
        tableSettings: {
          width: '100%',
          height: '483',
          colHeaders: ['选中该表格，请将数据复制到这里', '第一列', '第二列', '第三列'],
          data: [['', '', '', '']],
          fixedColumnsLeft: 1,
    ...
    methods: {
      ...
      submit() {
        ...
        async.eachOfSeries(this.bundle, (email, index, callback) => {
          const mailOptions = {
            from: this.form.user,
            to: email.to,
            subject: this.form.subject,
            html: this.generateHTML(this.form.content, email.table.head, email.table.rows),
          };

          transporter.sendMail(mailOptions, (error) => {
            if (error) return callback(new Error(`发送给“${mailOptions.to}”失败`));
      ...
      generateHTML(content, header, rows) {
        ...
        const thead = header.map(cell => `<th bgcolor="#335b8a" nowrap="nowrap" valign="top" width="120"><font color="#ffffff">${cell}</font></th>`).join('');
